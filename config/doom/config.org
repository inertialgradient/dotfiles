...
#+begin_src elisp
(after! org
  (defun do/config:fix-org-backgrounds ()
  ...
  (do/config:fix-org-backgrounds) ;; Apply immediately
  ;; Also apply after theme changes
  (add-hook! doom-load-theme #'do/config:fix-org-backgrounds)
  (add-hook! org-mode #'do/config:fix-org-backgrounds))
#+end_src
...
#+begin_src elisp
(defun do/config:set-preferred-theme (appearance)
...
(add-hook 'ns-system-appearance-change-functions #'do/config:set-preferred-theme)
#+end_src
...
#+begin_src elisp
(defun do/config:display:disable-line-numbers ()
  "Disable line numbers in the current buffer."
  (setq-local display-line-numbers nil))

(add-hook! '(org-mode-hook markdown-mode-hook) #'do/config:display:disable-line-numbers)
#+end_src
...
#+begin_src elisp
(defun do/config:keybindings:overrides ()
  (define-key help-quick-use-map (kbd "s-`") nil)
  (global-set-key (kbd "s-`") #'other-frame))
(add-hook 'emacs-startup-hook #'do/config:keybindings:overrides)
#+end_src
...
(defun do/config:disable-word-wrap ()
  (+word-wrap-mode -1))
(add-hook! org-mode #'do/config:disable-word-wrap)
(add-hook! org-agenda-mode #'do/config:disable-word-wrap)
#+end_src
...
(defun do/minutes-to-duration (minutes &optional duration-fmt)
  ...
  (let ((template (or format-str "%d:%02d")))
...
#+begin_src elisp
(defun do/config:keybindings:dired-jump ()
  (map! :n "-" #'dired-jump))
(add-hook! prog-mode #'do/config:keybindings:dired-jump)
(add-hook! text-mode #'do/config:keybindings:dired-jump)
#+end_src
...
#+begin_src elisp
(defun do/config:keybindings:dired ()
  (map! :map dired-mode-map
        :n "Z q" #'evil-quit
        :localleader
        :n "," #'casual-dired-tmenu))
(add-hook! dired-mode #'do/config:keybindings:dired)
#+end_src
...
(defun do/config:keybindings:hybrid ()
  (map! :map (org-mode-map evil-org-mode-map)
        :i "C-d" nil
        :i "C-k" nil))

(add-hook! org-mode :append #'do/config:keybindings:hybrid)
#+end_src
...
#+begin_src elisp
(defun do/config:flycheck-disable-lsp ()
  (setq flycheck-disabled-checkers '(lsp)))
(add-hook! prog-mode #'do/config:flycheck-disable-lsp)
(add-hook! text-mode #'do/config:flycheck-disable-lsp)
(add-hook! org-mode #'do/config:flycheck-disable-lsp)
#+end_src
...
#+begin_src elisp
(defun do/config:tab-to-fold-in-normal-state ()
  "Bind toggle-fold function to the <tab> key."
  (evil-local-set-key 'normal (kbd "<tab>") #'evil-toggle-fold))

(add-hook! prog-mode #'do/config:tab-to-fold-in-normal-state)
#+end_src
...
#+begin_src elisp
(defun do/disable-formatting ()
  (message "[apheleia] Disabling apheleia-mode.")
  (apheleia-mode -1))
(add-hook! latex-mode #'do/disable-formatting)
(add-hook! LaTeX-mode #'do/disable-formatting)
#+end_src
...
#+begin_src elisp
(defun do/pdf:refresh-buffer-display ()
...
(defun do/auto-revert-mode-pdf-view-refresh ()
  "Enable auto-revert-mode and refresh display for PDF buffers."
  (add-hook! auto-revert-mode #'do/pdf:refresh-buffer-display nil t))

(setq auto-revert-interval 1)
(setq auto-revert-verbose nil)

(add-hook! pdf-view-mode #'do/auto-revert-mode-pdf-view-refresh)
...
#+begin_src elisp
(defun do/config:keybindings:markdown ()
...
(after! markdown-mode
  ...
  (add-hook! markdown-mode :append #'do/config:keybindings:markdown))
#+end_src
...
#+begin_src elisp
(defun do/config:display:org-bullets ()
  (org-bullets-mode +1))

(add-hook! org-mode #'do/config:display:org-bullets)
#+end_src
...
(defun do/org:insert-heading-below ()
  ...
  (interactive)
  (progn
    (do/org:insert-heading-respect-content nil)
    (evil-insert 1)))
...
(after! org
  ...
  (defun do/config:keybindings:org-mode ()
  ...
  (add-hook! org-mode :append #'do/config:keybindings:org-mode))
#+end_src
...
#+begin_src elisp
(defun do/config:keybindings:web ()
  (map! :ni "C-<return>" #'emmet-expand-line))

(add-hook! '(web-mode-hook rjsx-mode-hook) #'do/config:keybindings:web)
#+end_src

Emmet should expand classes as =className= in JSX modes.

#+begin_src elisp
(defun do/config:emmet-expand-jsx-classname ()
  (setq emmet-expand-jsx-className? t))

(add-hook! '(rjsx-mode-hook typescript-tsx-mode-hook) #'do/config:emmet-expand-jsx-classname)
#+end_src
...
#+begin_src elisp
(defun do/config:keybindings:ruby ()
...
(add-hook! ruby-mode #'do/config:keybindings:ruby)
#+end_src

#+begin_src elisp
;; clear projectile-rails's keybinds
(defun do/config:keybindings:rails ()
...
(add-hook! ruby-mode #'do/config:keybindings:rails)
#+end_src
...
#+begin_src elisp
(defun do/config:keybindings:rspec ()
...
(add-hook! ruby-mode #'do/config:keybindings:rspec)
#+end_src
...
#+begin_src elisp
(defun do/config:flycheck-set-checker-ruby ()
  (flycheck-select-checker 'ruby-rubocop))
(add-hook! ruby-mode #'do/config:flycheck-set-checker-ruby)
#+end_src
...
#+begin_src elisp
(defun do/config:flycheck-set-checker-rust ()
  (flycheck-select-checker 'rust))
(add-hook! rust-ts-mode #'do/config:flycheck-set-checker-rust)
#+end_src
...
#+begin_src elisp
(defun do/config:keybindings:xwidget-webkit ()
...
(add-hook! xwidget-webkit-mode #'do/config:keybindings:xwidget-webkit)
#+end_src
...
#+begin_src elisp
(defun do/config:add-dot-to-word-chars ()
  "Adds underscore to the word chars syntax entry list."
  (modify-syntax-entry ?. "w"))

(defun do/config:add-underscore-to-word-chars ()
  "Adds underscore to the word chars syntax entry list."
  (modify-syntax-entry ?_ "w"))

(defun do/config:add-dash-to-word-chars ()
  "Adds underscore to the word chars syntax entry list."
  (modify-syntax-entry ?- "w"))

(defun do/config:add-to-word-char-list ()
  "Customize the word char list in prog and other modes."
  (add-hook! LaTeX-mode      #'do/config:add-dot-to-word-chars)
  (add-hook! latex-mode      #'do/config:add-dot-to-word-chars)
  (add-hook! coffee-mode     #'do/config:add-dash-to-word-chars)
  (add-hook! coffee-mode     #'do/config:add-underscore-to-word-chars)
  (add-hook! emacs-lisp-mode #'do/config:add-dash-to-word-chars)
  (add-hook! markdown-mode   #'do/config:add-underscore-to-word-chars)
  (add-hook! org-mode        #'do/config:add-underscore-to-word-chars)
  (add-hook! prog-mode       #'do/config:add-dash-to-word-chars)
  (add-hook! prog-mode       #'do/config:add-underscore-to-word-chars)
  (add-hook! python-mode     #'do/config:add-underscore-to-word-chars)
  (add-hook! restclient-mode #'do/config:add-underscore-to-word-chars)
  (add-hook! text-mode       #'do/config:add-underscore-to-word-chars)
  nil)
