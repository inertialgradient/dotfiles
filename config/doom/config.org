...
(defun +org-agenda-skip-not-todo-or-wip-or-not-today ()
  "Skip entries whose TODO state is not TODO/WIP, or that aren't scheduled for today."
  (let* ((state     (org-get-todo-state))
         (scheduled (org-get-scheduled-time (point)))
         (is-today  (and scheduled (>= (org-today) (time-to-days scheduled)))))
    (if (and (member state '("TODO" "WIP")) is-today)
        nil
...
(after! org
  ...
  (setq org-agenda-custom-commands
        `(("d" "Today (by priority)"
           ,(mapcar (lambda (p)
                      `(tags-todo ,(format "PRIORITY=\"%c\"" p)
                        ((org-agenda-overriding-header ,(format "Priority %c:" p))
                         (org-agenda-skip-function '+org-agenda-skip-not-todo-or-wip-or-not-today))))
