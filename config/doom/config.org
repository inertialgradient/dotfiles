...
(map! :desc "pwd vterm"     "s-'"   #'vterm-other-window
      ...
      "s-a"   #'org-agenda-today-by-priority
      "s-c"   #'evil-yank
      "s-f"   #'avy-goto-char-2
...
(after! org
  ...
  (setq org-agenda-custom-commands
        `(("d" "Today (by priority)"
           ,(append
             (mapcar (lambda (p)
                       `(tags ,(format "PRIORITY=\"%c\"" p)
                                   ((org-agenda-overriding-header ,(format "Priority %c:" p))
                                    (org-agenda-skip-function '+org-agenda-skip-done-or-canceled-or-not-today))))
                     (number-sequence org-priority-highest org-priority-lowest)))
           ((org-agenda-skip-function
             (lambda () (when (eq (org-agenda-count) 0) (org-agenda-skip))))))))
  nil)
#+end_src
...
(after! org
  (setq org-adapt-indentation t
        ...
        org-todo-keywords '((sequence "TODO(t)" "WIP(w)" "INBOX(i)" "|" "DONE(d)" "CANCELED(k)")
                            (sequence "[ ](T)" "[-](W)" "   (I)" "|" "[X](D)" "[~](K)"))
