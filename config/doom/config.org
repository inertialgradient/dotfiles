...
** select-enable-clipboard
 ...
 TODO: Provide a command for this and set to nil
#+begin_src elisp
...
(defun +new-emacs-instance ()
  ...
  (start-process
   "emacs" nil
   "/bin/zsh" "-c" "open ~/.dotfiles/scripts/Emacs.app"))
#+end_src

Some general purpose super-key keybindings, with meta-variants for terminal emacs.
...
(map! :desc "pwd vterm"     "s-'"   #'vterm-other-window
      ...
      "s-W"   #'+workspace/kill
      "s-C-w" #'(lambda () (interactive) (delete-frame nil t))
...
#+begin_src elisp
(defun org-agenda-today-by-priority ()
  (interactive)
  (org-agenda nil "d"))

(defun +org-agenda-skip-done-or-canceled-or-not-today ()
  "Skip entries that are DONE or CANCELED, or that aren't scheduled for today."
  (let* ((state     (org-get-todo-state))
         (scheduled (org-get-scheduled-time (point)))
         (is-today  (and scheduled (>= (org-today) (time-to-days scheduled)))))
    (if (and (not (member state '("DONE" "CANCELED"))) is-today)
        nil
...
(after! org-agenda
  (setq
   ...
   org-columns-default-format "%TODO(State) %50ITEM(Task) %6Effort(Effort){:} %10CLOCKSUM(Clocked) %3PRIORITY(Pri) %TAGS(Tags)"
  ...
  (set-face-attribute 'org-agenda-date-today nil :foreground "black" :height 1.0 :weight 'bold)
  (set-face-attribute 'org-agenda-date-weekend nil :foreground "DarkSteelBlue" :height 1.0 :weight 'bold)
  (set-face-attribute 'org-agenda-date nil :foreground "DarkSteelBlue" :height 1.0 :weight 'bold)
  (set-face-attribute 'org-agenda-structure nil :height 1.0 :weight 'bold)
  (set-face-attribute 'org-agenda-done nil :foreground "gray50" :strike-through t)
  (set-face-attribute 'org-scheduled-today nil :foreground "#556b72")
  (set-face-attribute 'org-scheduled-previously nil :foreground "#788484")
  (set-face-attribute 'org-scheduled nil :foreground "#788484")
  nil)
#+end_src
...
(after! org
  (setq org-agenda-files (list org-directory)
        ...
        +org-capture-projects-file "inbox.org"
